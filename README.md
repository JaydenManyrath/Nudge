...
├── render.yaml                  # Render Docker service + persistent disk config
...
Production builds the `Dockerfile` (gunicorn `wsgi:app`, binds `$PORT`) and deploys to Render through `render.yaml`.

Render deployment notes:

- `render.yaml` provisions one Docker web service and one persistent disk mounted at `/var/data`.
- `NUDGE_DB_PATH=/var/data/nudge.db` keeps SQLite data on the Render disk across deploys/restarts.
- Container startup runs `python -m scripts.init_db` before gunicorn so a fresh disk gets the schema.
- Render prompts for integration secrets marked `sync: false`: `ZOOM_CLIENT_ID`, `ZOOM_CLIENT_SECRET`, `ZOOM_SECRET_TOKEN`, `OPENAI_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and `SENDGRID_API_KEY`.
- `SECRET_KEY` is generated by Render. `RENDER_EXTERNAL_URL` is supplied by Render and is used for OAuth callback URLs unless `PUBLIC_BASE_URL`, `ZOOM_REDIRECT_URI`, or `GOOGLE_REDIRECT_URI` is set.
- Register these OAuth callback paths with the deployed domain: `/auth/zoom/callback` and `/auth/google/callback`.
